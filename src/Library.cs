...
public static class Library{
	...
	public static ResolvedImport BuildLibrary(string filename, (Delegate func, string description)[] functions){
		return new ResolvedImport(filename, null, null, functions.Select(t => BuildSingle(t.func, t.description)).Where(f => f != null).ToArray());
	}
	...
	public static ResolvedImport BuildLibrary(string filename, (string name, Delegate func, string description)[] functions){
		return new ResolvedImport(filename, null, null, functions.Select(t => BuildSingle(t.name, t.func, t.description)).Where(f => f != null).ToArray());
	}
	...
	public static FunctionExtStmt BuildSingle(string name, Delegate d, string desc){
		...
		return new FunctionExtStmt(name, parameters.Select((p, i) => p.Name ?? "arg" + i).ToArray(), compiledWrapper, desc);
	}
}